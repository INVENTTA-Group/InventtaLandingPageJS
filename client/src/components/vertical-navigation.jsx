...
import { NAVIGATION_ITEMS_VERTICALS, COMPANY_INFO } from "@/data/constants.js";
...
export default function Navigation() {
  ...
  /**
   * Handles logo clicks for redirecting to HomePage
   */
    ...
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-16">
          ...
          >
            <div className="w-48 h-auto" onClick={() => handleLogoClick()}>
              <SvgIconWithTag/>
            </div>
          </motion.div>
          ...
          <div className="hidden md:block">
            <div className="ml-10 flex items-baseline space-x-8">
              {NAVIGATION_ITEMS_VERTICALS.map((item, index) => (
                <motion.button
                  key={item.href}
        ...
        {isOpen && (
          ...
          >
            <div className="px-4 pt-2 pb-3 space-y-1">
              {NAVIGATION_ITEMS_VERTICALS.map((item) => (
                <button
                  key={item.href}
                  onClick={() => handleNavClick(item.href)}
